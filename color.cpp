...
sf::Color xorColor(int distance) {
    int value = std::min(255, distance);
    return sf::Color(value ^ 64, value ^ 32, value ^ 16, 255);
}

sf::Color modColor(int distance) {
    int value = std::min(255, distance);
    return sf::Color(value % 70, value % 8, value % 255, 255);
}

sf::Color andColor(int distance) {
    int value = std::min(255, distance);
    return sf::Color(value & 3, value  & 80, value & 150, 255);
}
...
sf::Color sinColor(int distance) {
    ...
    return sf::Color((sin(value * redPart) * 100) + gammaAdj, (sin(value * greenPart) * 100) - gammaAdj, (sin(value * bluePart) * 100) + gammaAdj, 255);
}

sf::Color tanColor(int distance) {
    int value = std::min(255, distance);
    return sf::Color(tan(value * 30.00) * 100, tan(value * 25.0) * 100, tan(value * 30.07) * 100, 255);
}